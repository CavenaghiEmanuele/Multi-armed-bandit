...
class PlainTSBernoulli(Agent):
    ...
    def __init__(self, id:str, actions: List[str], contexts:Dict):
        ...
        self._parameters = { 
            from_dict_to_json(context) : {action: [1, 1] for action in self._actions}
    ...
    def update_estimates(self, context:int, action: str, reward: int) -> None:
        if reward == 0:
            self._parameters[from_dict_to_json(context)][action][1] += 1  # Update beta
        else:  # Reward == 1
            self._parameters[from_dict_to_json(context)][action][0] += 1  # Update alpha
 
    def select_action(self, context:int, available_actions:List[str]) -> str:
        samples = {a:beta(
            a=self._parameters[from_dict_to_json(context)][a][0], 
            b=self._parameters[from_dict_to_json(context)][a][1]
            )
